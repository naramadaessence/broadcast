...
- **Bug Fix (Template Creation)**: Fixed an issue where creating a new video template via the UI threw an `OffsetInvalidError`. The root cause was twofold: (1) we previously used `multipart/form-data` which corrupted the video file boundaries, and (2) when switching back to raw binary, Meta's API would randomly drop or ignore the `file_offset` HTTP header (causing Meta to record 0 bytes and reject subsequent chunks). We now pass `file_offset` directly in the URL query string (`?file_offset=X`), which bypasses Meta's header-dropping bug while maintaining a perfectly uncorrupted raw binary payload.
**Why**: 
